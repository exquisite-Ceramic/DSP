...
|---|---|---|---|---|---|---|
| HYG-001 | P0 | CONSOLIDATE | Historical Step25–37 workflows above carry repository-wide pytest tails in addition to focused Step/domain guards; main CI shows broad-tail drift while representative focused gates remain meaningful. | Add one neutral current repository-wide regression owner; remove only duplicated broad pytest tails after the new gate is proven green/superset. Preserve focused tests, architecture guards, path filters, diff checks and Step36/37 Ruff-delta guards. | `repository-regression` green; structural workflow tests; root importlib + normal pytest green; modified historical focused workflows green. | OPEN |
