...
## Approved broad-tail workflow set

The originally frozen consolidation scope contained the 12 historical Step25–34/36/37 workflows. PR merge-readiness review then found four older workflows with the identical stale ownership pattern: focused/domain gates were green while a duplicated `Run full Python regression tests` tail failed. On 2026-09-12 the scope was explicitly extended only to those four workflows, preserving the same zero-product-behavior-change rule.

The approved repository-wide pytest-tail consolidation set is therefore:
...
|---|---|---|---|---|---|---|
| HYG-001 | P0 | CONSOLIDATE | Historical Step25–37 workflows and the four approved addendum workflows above carried repository-wide pytest tails in addition to focused Step/domain guards; focused checks remained meaningful while duplicated broad tails created false repository-level ownership and red CI. | Add one neutral current repository-wide regression owner; remove only duplicated broad pytest tails after the new gate is proven green/superset. Preserve focused tests, architecture guards, path filters, diff checks and Step36/37 Ruff-delta guards. | `repository-regression` green; structural workflow tests; root importlib + normal pytest green; every modified historical/domain focused workflow green. | **RESOLVED.** Canonical owner added in `37d2cf27c2ee816434ae9b6d214dcd244835bd14`; Step25–37 broad tails were removed without deleting focused ownership (Step25 `cd0362d1`, Step26 `df53cd1d`, Step27 `7c928028`, Step28 `dc41c573`, Step29 `5a5c0e94`, Step30 `29cae0a6`, Step31 `96964d5f`, Step32 `5d25323b`, Step33 `9b331801`, Step34 `e37c15ce`, Step36 `853eb4de`, Step37 `7a1482fd`). Ownership-source tests were aligned in `d39011c5` / `ecb3c5c2`; historical branch-only diff guards were scoped in `633ff6c4` / `66225dd2`. Merge-readiness addendum RED `9fc8c2ad2298a80e638dfc40139b798ac9dfc918` failed only because `operation-resolver.yml`, `semantic-mcp.yml`, `semantic-service.yml`, and `ifc43-semantic-provider.yml` still owned `Run full Python regression tests`; GREEN `afc5078a736afe25d077463a50de0bd7b629534c` removed only those four duplicated tails. On `afc5078`, `Repository regression` run `34677229282` and all 16 retained focused workflows were SUCCESS, including Operation Resolver `34677229311`, Semantic MCP `34677229211`, Semantic Service `34677229278`, and IFC4.3 `34677229354`. Coverage moved; it was not deleted. |
| HYG-002 | P0 | FIX | Reusable test builders were exposed through `conftest.py` and sibling `test_*.py` modules, making collection depend on incidental test-module layout. | Move reusable helpers into explicit `_support.py` modules; leave `conftest.py` for pytest fixture wiring only; production code unchanged. | Affected domains pass in importlib and normal modes; root repository modes remain green. | **RESOLVED.** Shared test support was extracted beginning with `8d29f5d5018b14a815d4fd7b6f35f1d4172e81fc` and `e78c3a38b4c679be7968b5967dd4677df5ecce9e`, then completed across readiness/reconciliation/materialized coordination support (`53f3f923`, `55aa3acd`, `f201009e`, `10825f07`, `43e6a96d`) and normal-mode package/import stabilization (`7886dc55`, `79f758ba`, `83d79717`). Final repository verification on `afc5078` passed both `python -m pytest --import-mode=importlib -q` and `python -m pytest -q`; no production source was edited. |
...
| HYG-010 | P1 | DEFER | `jsonschema.RefResolver` warning exists, but contract-equivalent migration has not been proven in the frozen hygiene scope. | No runtime validator migration here. | Diff proves validation production path unchanged. | **DEFERRED.** RefResolver migration was not proven contract-equivalent. Baseline-to-`afc5078` diff contains no production validator or contracts/schema edit. |
...
| HYG-012 | P1 | DOCUMENT | Phase H workflow is focused-only and therefore not evidence for the historical broad-tail consolidation problem. | Retain Phase H workflow structure; document separate focused ownership. | Structural workflow test verifies Phase H focused steps remain; Phase H CI green. | **DOCUMENTED / VERIFIED.** Phase H was explicitly excluded from the approved broad-tail consolidation set and its workflow YAML was retained as focused-only. On `afc5078`, Phase H run `34677229291` completed SUCCESS; the only Phase H code change in this stage was the stale Python source-text assertion repair recorded by HYG-014. |
| HYG-013 | P1 | DOCUMENT | gRPC generator/transport tooling remains referenced by transport conformance/dev workflows; it is not proven dead. | Retain tooling and document KEEP rationale; no removal. | Current workflow/tool references remain present; source-tree diff shows no tooling removal. | **DOCUMENTED / RETAINED.** gRPC generator/transport tooling was not removed or rewritten because current conformance/dev references still require it. Baseline-to-`afc5078` diff contains no removal of the transport generator/tooling surfaces. |
| HYG-014 | P0 | FIX | Phase H on the baseline failed one Revit architecture source-contract assertion: the test expected `revisions.Get(document)` while the handler now reads `revisions.Get(uiDocument.Document)` after the executor boundary moved to `UIDocument`; Revit Core remained green. | Update only the stale Python architecture textual assertion. Do not edit `RevitExternalEventHandler.cs`, `DocumentRevisionTracker.cs`, or Host behavior. | `tests/revit/test_revit_architecture.py` green; Revit Core green; Phase H workflow green. | **RESOLVED.** `bb7f523ff0fe03a065d496d4f017e6c9ebff8724` updated only the stale Python architecture assertion. Revit production files were untouched. On `afc5078`, Phase H run `34677229291` and canonical Revit Core were SUCCESS. |

## Final verification evidence

- Canonical repository owner: `Repository regression` run `34677229282` on `afc5078a736afe25d077463a50de0bd7b629534c` — SUCCESS for importlib root pytest, normal-mode root pytest, Ruff E/F/I baseline-delta, and Revit Core.
- Phase H: run `34677229291` on the same head — SUCCESS.
- Phase I: run `34677229245` on the same head — SUCCESS for the offline job; the real dual-Host job remains a separate manual/self-hosted gate and is not substituted by offline CI.
- Historical focused workflows Step25–34/36/37 on the same head — all SUCCESS.
- Approved merge-readiness addendum workflows on the same head — Operation Resolver `34677229311`, Semantic MCP `34677229211`, Semantic Service `34677229278`, IFC4.3 semantic provider `34677229354` — all SUCCESS with focused/domain gates retained and duplicated full-repository tails removed.
- Additional triggered focused workflows, including Metro, Enterprise, and Step22, also completed SUCCESS on the same head.
- Source-tree boundary: `main@d2d1621b30f87506c62adb2d12f73d387821ef78...afc5078` contains no product/runtime changes under `platform/**/src/**`, Host/plugin production runtime, provider runtime, or contracts/schema.
