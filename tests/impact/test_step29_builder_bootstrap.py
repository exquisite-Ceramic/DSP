...
def test_root_operation_is_materialized_from_exact_canonical_contract() -> None:
    builder = design_changeset.ChangeSetBuilder()
...
def test_material_argument_swap_cannot_reuse_old_impact_analysis() -> None:
    ...
    _assert_code(
        ...
        lambda: builder.build(_request(bound_evidence=changed, impact=old_impact)),
    )
...
def test_canonical_arguments_are_validated_against_exact_step23_schema() -> None:
    ...
    impact = replace(
        impact,
        bound_operation_fingerprint=invalid.bound_operation_fingerprint,
    )
    builder = design_changeset.ChangeSetBuilder()

    _assert_code(
        "CHANGESET_ARGUMENTS_INVALID",
        lambda: builder.build(_request(bound_evidence=invalid, impact=impact, scope=_scope(impact))),
    )
...
def test_bound_targets_must_equal_step27_direct_targets() -> None:
    ...
    impact = replace(
        impact,
        bound_operation_fingerprint=changed.bound_operation_fingerprint,
    )
    builder = design_changeset.ChangeSetBuilder()

    _assert_code(
        "CHANGESET_TARGET_MISMATCH",
        lambda: builder.build(_request(bound_evidence=changed, impact=impact, scope=_scope(impact))),
    )
...
def test_root_effects_must_be_fully_covered_by_explicit_scope_rules() -> None:
    ...
    _assert_code(
        ...
        lambda: builder.build(_request(impact=impact, scope=narrowed)),
    )
