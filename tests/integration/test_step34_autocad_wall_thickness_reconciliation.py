"""Step34: provider-neutral AutoCAD wall-thickness reconciliation proof."""
...
def _build_transaction() -> Step34Transaction:
    ...
    environment = SemanticEnvironmentBinding("ENV-STEP34", "env-hash-step34")
    ...
    boundary = bind_changeset(scope, changeset.changeset_hash, "SCOPE-STEP34")
    runtime = HostRuntimeRef("autocad", "HOST-STEP34-A", "DOC-STEP34")
    ...
    return Step34Transaction(changeset, boundary, plan, plan.execution_slices[0])
...
def _build_binding_set(execution_slice: ExecutionSlice) -> ProviderBindingSet:
    ...
    draft = ProviderBinding(
        ...
        provider_arguments={"handles": ["A31"], "thickness_mm": 300.0},
        provider_preconditions=(),
        native_binding_metadata={"step34": "wall-thickness"},
    ...
    return ProviderBindingSet(
        ...
        execution_slice_id=execution_slice.execution_slice_id,
        execution_slice_hash=execution_slice.execution_slice_hash,
        provider_execution_snapshot_id="PXS-STEP34",
...
def _admit_authority(
    ...
    approval = service.consume_approval(
        ApprovalConsumptionRequest(
            ...
            consumed_at="2026-08-30T20:10:00Z",
        )
    )
    grant = service.issue_execution_grant(
        ExecutionGrantRequest(
            ...
            issued_at="2026-08-30T20:20:00Z",
        )
    )
...
def _signed_change(*, extra_scope: bool = False) -> reconciliation.ActualChange:
    aspects = (
        (CanonicalAspect.PROPERTIES, CanonicalAspect.GEOMETRY)
        if extra_scope
        else (CanonicalAspect.PROPERTIES,)
    )
    draft = reconciliation.ActualChange(
        change_kind=reconciliation.ActualChangeKind.MODIFY,
        semantic_id="WALL-001",
    ...
    return replace(
        draft,
        actual_change_hash=reconciliation.compute_actual_change_hash(draft),
    )
...
def _signed_delta(
    ...
    change: reconciliation.ActualChange,
) -> reconciliation.ActualDelta:
    draft = reconciliation.ActualDelta(
        actual_delta_id="AD-STEP34",
...
def _assigned_tasks(stored, transaction: Step34Transaction):
...
def _signed_bundle(
    ...
    *,
    ...
    projection = SemanticProjectionRef(
        projection_id="PROJ-STEP34",
        projection_hash=canonical_hash(
            {"step34": "projection", "width_mm": width_mm}
        ),
        semantic_model_version="dsp-core+ifc43+enterprise-step34",
        provider_set_hash=canonical_hash({"step34": "providers"}),
        mapping_profile_set_hash=canonical_hash({"step34": "mappings"}),
        normalized_fact_batch_hash=canonical_hash(
            {"step34": "facts", "width_mm": width_mm}
        ),
    )
    snapshot = SemanticSnapshot(
        snapshot_id="PS-STEP34-POST",
        kind=SnapshotKind.PLANNING,
        project_id=changeset.project_id,
        freshness_contract_id="FC-STEP34-POST",
        freshness_contract_hash=canonical_hash({"step34": "freshness"}),
...
def _drive_to_scope(
    ...
    saga = service.reserve_slice_admission(
        ...
        reserved_at="2026-08-30T21:00:00Z",
    )
    ...
    saga = service.record_host_commit(
        ...
        delta,
        expected_revision=saga.saga_revision,
        committed_at="2026-08-30T21:01:00Z",
    )
...
def _verification_request(
    ...
    *,
    ...
    return reconciliation.SemanticVerificationRequest(
        ...
        verified_at="2026-08-30T21:02:00Z",
    )


def test_step34_wall_thickness_reaches_succeeded_only_after_scope_and_semantic_verification() -> None:
    ...
    delta = _signed_delta(transaction, authority, change)
    service = _service()

    native_target = binding_set.bindings[0].native_targets[0]
    assert native_target.native_kind == "LWPOLYLINE"
    assert native_target.native_id == "A31"
...
def test_step34_wrong_reconstructed_width_becomes_verify_failed_not_succeeded() -> None:
    transaction = _build_transaction()
    binding_set = _build_binding_set(transaction.execution_slice)
    authority = _admit_authority(transaction, binding_set)
    delta = _signed_delta(transaction, authority, _signed_change())
    service = _service()
...
def test_step34_extra_actual_aspect_becomes_scope_breach_not_succeeded() -> None:
    transaction = _build_transaction()
    binding_set = _build_binding_set(transaction.execution_slice)
    authority = _admit_authority(transaction, binding_set)
    delta = _signed_delta(
        transaction,
        authority,
        _signed_change(extra_scope=True),
    )
    service = _service()
    ...
    assert {violation.code for violation in scope_result.violations} == {
    ...
    assert saga.status is not reconciliation.ExecutionSagaStatus.SUCCEEDED
