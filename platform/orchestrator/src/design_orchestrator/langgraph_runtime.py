...
class LangGraphWorkflowRuntime(WorkflowOrchestratorPort):
    ...
    def resume(
        ...
        try:
            ...
            if resume_mode == "human" and checkpoint.pending_interaction is not None:
                # 命令通过 correlation/shape 校验后才记录 artifact identity。
                # stale/mismatch/invalid 不记录 artifact preflight。
            ...
            if checkpoint_version is None and resume_mode == "human":
                ...
                try:
                    ...
                    # ``as_node`` 只声明 prepare 已完成；后继 await node 建立真实 interrupt。
                    # 不重新执行 prepare node，避免生成新的随机 pause_id。
